    ...
    return (
        ...
        >
            <div className="min-w-0">
                <p className="text-sm font-semibold text-primary truncate" title={getDisplayName(user)}>{getDisplayName(user)}</p>
                <p className="text-xs text-secondary truncate" title={subtitleParts.join(' · ')}>{subtitleParts.join(' · ')}</p>
            </div>
                    ...
                    {pendingRequests.map((request, index) => {
                        ...
                        return (
                            <GlassCard key={requestId ?? index} theme={theme} className="p-4 flex items-center justify-between gap-4">
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-primary truncate" title={getDisplayName(user)}>{getDisplayName(user)}</p>
                                    <p className="text-xs text-secondary truncate" title={[user?.username, user?.email, user?.phone].filter(Boolean).join(' · ')}>
